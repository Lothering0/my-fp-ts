...
export const flat: {
  <A>(asyncOption: AsyncOption<AsyncOption<A>>): AsyncOption<A>
} = Monad.flat
...
export const andThen: {
  ...
  ): (selfAsyncOption: AsyncOption<unknown>) => AsyncOption<A>
} = Monad.andThen
...
export const setTo: {
  <N extends DoObjectKey, A, B>(
    ...
    b: B,
  ): (asyncOption: AsyncOption<A>) => AsyncOption<DoObject<N, A, B>>
} = Monad.setTo
...
export const flipApplyTo: {
  <N extends DoObjectKey, A, B>(
    name: Exclude<N, keyof A>,
    asyncOption: AsyncOption<(a: A) => B>,
  ): (selfAsyncOption: AsyncOption<A>) => AsyncOption<DoObject<N, A, B>>
} = Monad.flipApplyTo
...
export const concurrently: {
  <N extends DoObjectKey, B>(
    asyncOption: AsyncOption<B>,
  ): <A>(selfAsyncOption: AsyncOption<A>) => AsyncOption<DoObject<N, A, B>>
} = asyncOption => selfAsyncOption => () =>
  Promise.all([toPromise(selfAsyncOption), toPromise(asyncOption)]).then(
    ([ma, mb]) => pipe(mb, Option.andThen(ma as any)),
  )
...
} = (name, asyncOption) => selfAsyncOption => () =>
  Promise.all([toPromise(selfAsyncOption), toPromise(asyncOption)]).then(
    ([ma, mb]) => Option.bind(name, mb)(ma),
  )
