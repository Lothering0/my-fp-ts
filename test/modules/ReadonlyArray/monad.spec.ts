import { Number, Array, pipe } from '../../../src'
...
describe('gen', () => {
  it('`$` function should accept both lazy and eager versions of a readonly array', () => {
    const f = jest.fn()
    const identity: <A>(a: A) => A = jest.fn(a => a)

    const FROM = 1
    const TO = 5
    const ma = Array.gen(function* ({ $, where }) {
      f()
