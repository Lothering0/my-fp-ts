...
export const getOrElse: {
  <B, E>(
    onFailure: (e: E) => B,
  ): <A>(syncResult: SyncResult<A, E>) => Sync<A | B>
} = _SyncResult.getOrElse

export const orElse: {
  <B, E>(
    onFailure: SyncResult<B, E>,
  ): <A>(syncResult: SyncResult<A, unknown>) => SyncResult<A | B, E>
} = _SyncResult.orElse

export const orElseSucceed: {
  <B>(
    onFailure: B,
  ): <A>(syncResult: SyncResult<A, unknown>) => SyncResult<A | B>
} = _SyncResult.orElseSucceed

export const orElseFail: {
  <E>(onFailure: E): <A>(syncResult: SyncResult<A, unknown>) => SyncResult<A, E>
} = _SyncResult.orElseFail

export const catchAll: {
  <B, E1, E2>(
    onFailure: (e: E1) => SyncResult<B, E2>,
  ): <A>(syncResult: SyncResult<A, E1>) => SyncResult<A | B, E2>
